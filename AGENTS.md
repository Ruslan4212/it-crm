...
- Frontend runs on HTTPS (self-signed) for microphone/voice support: `https://localhost:5173/`
- Regenerate certs if needed: `make certs` or `scripts/gen-certs.sh`

## HTTPS
- Dev: self-signed certs in `certs/` (generated by `scripts/gen-certs.sh`)
- Vite dev server (port 5173) uses HTTPS automatically
- Production: replace with real certs (Let's Encrypt) + update nginx.conf
